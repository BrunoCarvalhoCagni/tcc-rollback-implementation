...
public class Cat  {
    ...
    public int currentActionFrameCount { get { return catData.actions[currentActionID].frameCount; } }
    private bool isActionEnd { get { return (currentActionFrame >= catData.actions[currentActionID].frameCount); } }
    public bool isAlwaysCancelable { get { return catData.actions[currentActionID].alwaysCancelable; } }
    ...
    /// <summary>
    /// Setup cat at the start of battle
    /// </summary>
    /// <param name="catData"></param>
    /// <param name="startPosition"></param>
    /// <param name="isPlayerOne"></param>
	
    public void SetupBattleStart(CatData catData, Vector2 startPosition, bool isPlayerOne)
        {
            ...
            vitalHealth = 1;
            guardHealth = catData.startGuardHealth;
            hasWon = false;
    ...
    /// </summary>
        ...
        {
            // Decrease sprite shake count and swap +/- (used by BattleGUI for cat sprite position)
            ...
            // If cat is in hit stun then the action frame stay the same
            ...
            if (isActionEnd)
            {
                if(catData.actions[currentActionID].isLoop)
                {
                    currentActionFrame = catData.actions[currentActionID].loopFromFrame;
                }
            }
        }
    ...
    /// <summary>
        ...
        public void UpdateMovement()
        {
            ...
            {
                position.x += catData.forwardMoveSpeed * sign * Time.deltaTime;
                return;
            }
            else if (currentActionID == (int)CommonActionID.BACKWARD)
            {
                position.x -= catData.backwardMoveSpeed * sign * Time.deltaTime;
                return;
            }

            // Position changes from action data
            var movementData = catData.actions[currentActionID].GetMovementData(currentActionFrame);
        ...
        public void NotifyAttackHit(Cat damagedCat, Vector2 damagePos)
        {
        ...
        public DamageResult NotifyDamaged(AttackData attackData, Vector2 damagePos)
        {
            ...
            if (currentActionID == (int)CommonActionID.BACKWARD
                || catData.actions[currentActionID].Type == ActionType.Guard) // if in blocking motion, automatically block next attack
            {
                if (isGuardBreak)
                {
                    SetCurrentAction(attackData.guardActionID);
                    reserveDamageActionID = (int)CommonActionID.GUARD_BREAK;
                    SoundManager.Instance.playCatSE(catData.actions[reserveDamageActionID].audioClip, isFaceRight, position.x);
        ...
        {
            if(!catData.attackData.ContainsKey(attackID))
            {
                Debug.LogWarning("Attack hit but AttackID=" + attackID + " is not registered");
                return true;
            }

            if (currentActionHitCount >= catData.attackData[attackID].numberOfHit)
                return false;
        ...
        public AttackData getAttackData(int attackID)
        {
            ...
            return catData.attackData[attackID];
        }
        ...
        public int GetHitStunFrame(DamageResult damageResult, int attackID)
        {
            if(damageResult == DamageResult.Guard)
                return catData.attackData[attackID].guardStunFrame;
            else if (damageResult == DamageResult.GuardBreak)
                return catData.attackData[attackID].guardBreakStunFrame;

            return catData.attackData[attackID].hitStunFrame;
        }
        ...
        public bool RequestAction(int actionID, int startFrame = 0)
        {
            ...
            if (catData.actions[currentActionID].alwaysCancelable)
            {
                SetCurrentAction(actionID, startFrame);
                return true;
            }
            ...
            {
                foreach (var cancelData in catData.actions[currentActionID].GetCancelData(currentActionFrame))
                {
        ...
        {
            var motionData = catData.actions[currentActionID].GetMotionData(currentActionFrame);
            if (motionData == null)
                return null;

            return catData.motionData[motionData.motionID].sprite;
        }
        ...
        private void SetCurrentAction(int actionID, int startFrame = 0)
        {
            ...
            if(catData.actions[currentActionID].audioClip != null)
            {
                if (currentActionID == (int)CommonActionID.GUARD_BREAK)
                    return;

                SoundManager.Instance.playCatSE(catData.actions[currentActionID].audioClip, isFaceRight, position.x);
            }
        }
        ...
        private bool CheckSpecialAttackInput()
        {
            ...
            for (int i = 1; i < catData.specialAttackHoldFrame; i++)
            {
        ...
        private bool CheckForwardDashInput()
        {
            ...
            for(int i = 1; i < catData.dashAllowFrame; i++)
            {
                ...
                {
                    for(int j = i + 1; j < i + catData.dashAllowFrame; j++)
                    {
        ...
        /// <summary>
        /// Copy data from current action and convert relative box position with cat position
        /// </summary>
        private void ApplyCurrentActionData()
        {
            ...
            foreach (var hitbox in catData.actions[currentActionID].GetHitboxData(currentActionFrame))
            {
                var box = new Hitbox();
            ...
            foreach (var hurtbox in catData.actions[currentActionID].GetHurtboxData(currentActionFrame))
            {
                var box = new Hurtbox();
                Rect rect = hurtbox.useBaseRect ? catData.baseHurtBoxRect : hurtbox.rect;
                box.rect = TransformToFightRect(rect, position, isFaceRight);
                hurtboxes.Add(box);
            }

            var pushBoxData = catData.actions[currentActionID].GetPushboxData(currentActionFrame);
            pushbox = new Pushbox();
            Rect pushRect = pushBoxData.useBaseRect ? catData.basePushBoxRect : pushBoxData.rect;
            pushbox.rect = TransformToFightRect(pushRect, position, isFaceRight);
        }

        /// <summary>
        /// Convert relative box position with current cat position
        /// </summary>
